...
class NSRU_Meetings {
	...
	public function meta_box_callback( $post ) {
		...
		$day_select   = '<select name="meeting_date" id="meeting_date"><option value="">Select a meeting date</option>';
		for ( $the_date = $start_date; $the_date != $end_date; ) {
			$selected    = $the_date === $meeting_date ? 'selected' : '';
			$day_of_week = date ( 'l', strtotime( $the_date ) );
			$day_select .= '<option value="' . $the_date . '"' . $selected . '>' . $day_of_week . '</option>';
		...
		<table class="form-table">
			<tr>
				...
				<td><?php echo $day_select; ?></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Start Time:', 'nsru-options' ); ?></th>
				<td><?php echo $this->create_time_select( 'hrs', 'start_time_hrs', esc_attr( $start_time_hrs ) ); ?> : <?php echo $this->create_time_select( 'mins', 'start_time_mins', esc_attr( $start_time_mins ) ); ?></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Stop Time:', 'nsru-options' ); ?></th>
				<td><?php echo $this->create_time_select( 'hrs', 'stop_time_hrs', esc_attr( $stop_time_hrs ) ); ?> : <?php echo $this->create_time_select( 'mins', 'stop_time_mins', esc_attr( $stop_time_mins ) ); ?></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Topic:', 'nsru-options' ); ?></th>
				<td><input type="text" id="topic" name="topic" class="width-100" value="<?php echo esc_attr( $topic ); ?>" /></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Speakers:', 'nsru-options' ); ?></th>
				<td><input type="text" id="speakers" name="speakers" class="width-100" value="<?php echo esc_attr( $speakers ); ?>" /></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Hosted By:', 'nsru-options' ); ?></th>
				<td><input type="text" id="hosted_by" name="hosted_by" class="width-100" value="<?php echo esc_attr( $hosted_by ); ?>" /></td>
			</tr>
			<tr>
				<th scope="row"><?php echo __( 'Rooms:', 'nsru-options' ); ?></th>
				<td><?php echo $rooms_select; ?></td>
			</tr>
		</table>
	...
	public function save_meta_box_data( $post_id) {
		...
		$start_date = $_POST['meeting_date'] . ' ' . $_POST['start_time_hrs'] . ':' . $_POST['start_time_mins'];
		...
		switch ($column) {
			case 'meeting_date':
				...
				if ( false === $meeting_date ) {
					...
					$meeting_date = date( 'l F j, Y', $meeting_date );
				}
				echo esc_attr( $meeting_date );
				break;
			case 'start_time':
			...
			case 'stop_time':
				...
				echo esc_attr( $stop_time );
				break;
			case 'topic':
				echo esc_attr( $meta[0]['topic'] );
				break;
			case 'speaker':
				echo esc_attr( $meta[0]['speakers'] );
				break;
			case 'hosted_by':
				echo esc_attr( $meta[0]['hosted_by'] );
				break;
			case 'room':
